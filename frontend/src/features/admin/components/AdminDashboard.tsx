  ...
  return (
    <AdminShell activeSection="dashboard">
      ...
      <section className="mt-8 grid grid-cols-2 gap-4 xl:grid-cols-4">
        {metrics.map(([label, value]) => <article className="rounded border bg-white p-4 shadow-[3px_3px_2.5px_1px_#1b3a6b]" key={label}><p className="text-xs text-[#707070]">{label}</p><p className="mt-3 text-3xl font-bold text-[#1b3a6b]">{value}</p></article>)}
      </section>
      <section className="mt-8 rounded border bg-white p-5 shadow-[3px_3px_2.5px_1px_#1b3a6b]">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-3">
          {['SUBMITTED', 'IN_PROGRESS', 'RESOLVED'].map((status) => <article className="rounded bg-[#edf4ff] p-4" key={status}><p className="text-xs">{status.replaceAll('_', ' ')}</p><p className="mt-2 text-2xl font-bold">{concerns.data?.data.filter((item) => item.status === status).length ?? 0}</p></article>)}
        </div>
      </section>
    </AdminShell>
  )
